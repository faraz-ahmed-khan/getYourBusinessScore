...
import type { StoredScoreResult } from '@/lib/gybs-result-helpers';

function levelBadge(level: 1 | 2 | 3): { text: string; className: string } {
  if (level === 1) return { text: 'Level 1 — Foundational Lane', className: 'bg-gybs-accent/15 text-gybs-ink border-gybs-accent/40' };
  if (level === 2) return { text: 'Level 2 — Guided Lane', className: 'bg-gybs-blue/10 text-gybs-navy border-gybs-blue/30' };
  return { text: 'Level 3 — Opportunity Lane', className: 'bg-gybs-success/15 text-gybs-ink border-gybs-success/40' };
}

export function ResultsClient() {
  const [data, setData] = useState<StoredScoreResult | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      const parsed = JSON.parse(raw) as StoredScoreResult;
      setData(parsed);
  ...
  const badge = useMemo(() => (data ? levelBadge(data.level) : null), [data]);

  if (!data) {
    return (
      <div className="mx-auto max-w-lg px-4 py-24 text-center">
        <p className="text-gybs-muted">No results found. Complete the assessment to see your Business Readiness Score.</p>
      ...
      <div className="mx-auto max-w-content px-4 pt-10 md:px-6">
            ...
            <div className="relative mx-auto mt-8 h-32 w-48">
              <svg viewBox="0 0 120 60" className="h-full w-full">
                ...
                <path
                  ...
                  strokeLinecap="round"
                  strokeDasharray="157"
                  strokeDashoffset={157 - (157 * data.finalScore) / 100}
                  style={{ transition: 'stroke-dashoffset 1.5s cubic-bezier(0.4,0,0.2,1)' }}
                />
              </svg>
            </div>
            {badge && (
              <span className={`mt-6 rounded-full border px-4 py-2 text-sm font-semibold ${badge.className}`}>{badge.text}</span>
            )}
          </div>
        </div>
        ...
        <div className="mx-auto mt-10 max-w-[800px] rounded-xl border border-gybs-border bg-white p-6 shadow-gybs-card md:p-8">
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { Icon: Layers, label: 'Readiness Level', value: `Level ${data.level}` },
              { Icon: Navigation, label: 'Your Lane', value: data.lane },
              { Icon: Package, label: 'Assigned Pack', value: data.packName },
        ...
        <section className="mx-auto mt-12 max-w-[800px]">
          ...
          <ol className="mt-6 space-y-4">
            {data.upgradeSteps.map((s, i) => (
              <li key={i} className="flex gap-4 rounded-xl border border-gybs-border bg-white p-4">
                ...
                <div className="flex flex-1 items-start gap-2">
                  {s.done ? <Check className="h-5 w-5 text-gybs-success" strokeWidth={2.5} /> : <span className="w-5" />}
                  <span className={`text-sm ${s.done ? 'text-gybs-muted line-through' : 'text-gybs-body'}`}>{s.label}</span>
                </div>
              </li>
            ))}
          </ol>
        ...
        <section className="mx-auto mt-16 max-w-[800px]">
          <h2 className="text-center text-2xl font-bold text-gybs-ink md:text-3xl">Continue Your Readiness Journey</h2>
        ...
        >
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="rounded-xl border border-white/20 bg-white p-6 text-gybs-ink shadow-lg">
              <span className="inline-block rounded-full bg-slate-200 px-3 py-1 text-xs font-bold text-gybs-muted">CURRENT</span>
              ...
              <button type="button" disabled className="mt-6 w-full rounded-lg border-2 border-gybs-border py-3 text-sm font-semibold text-gybs-muted">
                You&apos;re on Free
              </button>
            </div>
            <div className="rounded-xl border border-white/20 bg-white p-6 text-gybs-ink shadow-lg">
              <span className="inline-block rounded-full bg-gybs-blue/15 px-3 py-1 text-xs font-bold text-gybs-blue">BASIC</span>
            ...
            <div className="relative rounded-xl border-2 border-gybs-accent bg-white p-6 text-gybs-ink shadow-xl lg:-translate-y-1">
              <span className="inline-block rounded-full bg-gybs-accent/20 px-3 py-1 text-xs font-bold text-gybs-ink">RECOMMENDED</span>
